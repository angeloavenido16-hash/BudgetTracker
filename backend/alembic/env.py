...
def _strip_async_driver(url: str) -> str:
    """Strip the async driver suffix (e.g. +asyncpg, +aiosqlite) for sync use.

    Alembic's offline mode needs a sync-compatible URL; the runtime driver info
    is irrelevant for DDL rendering.
    """
    import re
...
def run_migrations_offline() -> None:
    ...
    `alembic upgrade head --sql` renders DDL even without the async driver installed.
    """
    url = _strip_async_driver(config.get_main_option("sqlalchemy.url"))
    context.configure(
        url=url,
