"""Transactions router — CRUD (user-scoped).
...
async def list_transactions(
    ...
    stmt = (
        ...
        .where(TxnModel.user_id == user_id)
...
async def create_transaction(
    ...
    txn = TxnModel(**body.model_dump(), user_id=user_id)
    session.add(txn)
...
async def update_transaction(
    ...
    txn = await session.scalar(
        select(TxnModel).where(
            TxnModel.id == txn_id, TxnModel.user_id == user_id
        )
    )
    if txn is None:
