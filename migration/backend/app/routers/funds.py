"""Funds router — CRUD + per-fund summaries.
...
router = APIRouter(
    prefix="/funds", tags=["funds"], dependencies=[Depends(get_current_user)]
)


async def _get_or_404(session: AsyncSession, fund_id: int) -> FundModel:
    fund = await session.get(FundModel, fund_id)
    if fund is None:
...
async def list_funds(
    ...
    stmt = select(FundModel).order_by(
        # SQLite puts NULL cutoff_date LAST under DESC; Postgres puts them FIRST.
        # Force NULLS LAST so the web ordering matches the desktop app.
...
# NOTE: declared BEFORE /{fund_id} so "summaries" isn't parsed as an id.
@router.get("/summaries", response_model=dict[int, FundSummary])
async def fund_summaries(session: AsyncSession = Depends(get_session)):
...
async def get_fund(fund_id: int, session: AsyncSession = Depends(get_session)):
    return await _get_or_404(session, fund_id)


@router.get("/{fund_id}/summary", response_model=FundSummary)
async def fund_summary(fund_id: int, session: AsyncSession = Depends(get_session)):
    fund = await _get_or_404(session, fund_id)
    txns = await queries.fetch_transactions(session)
...
async def create_fund(body: FundCreate, session: AsyncSession = Depends(get_session)):
    fund = FundModel(**body.model_dump())
    session.add(fund)
    try:
...
async def update_fund(
    fund_id: int, body: FundUpdate, session: AsyncSession = Depends(get_session)
):
    fund = await _get_or_404(session, fund_id)
...
async def delete_fund(fund_id: int, session: AsyncSession = Depends(get_session)):
    fund = await _get_or_404(session, fund_id)
    await session.delete(fund)  # ON DELETE CASCADE removes its transactions
    await session.commit()
