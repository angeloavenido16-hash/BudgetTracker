...
export default function Dashboard() {
  ...
  // Missing Expenses: red if positive (unaccounted), green if ≤ 0. Uses present.
  const missing = present.data?.missing_expenses ?? 0;
  const bpi = present.data?.bpi_balance ?? 0;
  const missingColor = missing > 0 ? "var(--red)" : "var(--green)";
      ...
      <div className="present-band">
        ...
        <div className="present-cards">
          ...
          <div className="stat-card">
            ...
            <div className="value" style={{ color: missingColor }}>
              {peso(missing)}
            </div>
          </div>
        </div>
